...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  """
  if model_architecture == 'dscnn_bnn_2dscv2':
      return dscnn_bnn_2dscv2(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_h5':
      return dscnn_bnn_h5(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_3dsc':
      return dscnn_bnn_3dsc(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_2dsc':
      return dscnn_bnn_2dsc(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_1dsc':
      return dscnn_bnn_1dsc(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_1dscv2':
      return dscnn_bnn_1dscv2(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC512_4_tnn':
      return FC512_4_tnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC512_4_bnntnn':
      return FC512_4_bnntnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC512_4_bnn':
      return FC512_4_bnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'cnn_bnntnn':
      return cnn_bnntnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'cnnv1':
      return cnnv1(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'twn_3bit_theta':
      return twn_3bit_theta(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_8bit':
      return dscnn_bnn_8bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_6bit':
      return dscnn_bnn_3bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_4bit':
      return dscnn_bnn_3bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn_3bit':
      return dscnn_bnn_3bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC512_4':
      return FC512_4(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC256_4':
      return FC256_4(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC128_4':
      return FC128_4(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'cnn_bnn':
      return cnn_bnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'cnn_tnn':
      return cnn_tnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'cnn_twn_3bit':
      return cnn_twn_3bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC128_tnn':
      return FC128_tnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC128_bnn':
      return FC128_bnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'FC128_twn_3bit':
      return FC128_twn_3bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnn':
      return dscnn_bnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_bnntnn':
      return dscnn_bnntnn(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscnn_tnn2':
      return dscnn_tnn2(fingerprint_input, model_settings,
                      is_training)
  # if model_architecture == 'dscnn_tnn':
  #     return dscnn_tnn(fingerprint_input, model_settings,
  #                     is_training)
  if model_architecture == 'twn_3bit':
      return twn_3bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'twn_2bit':
      return twn_2bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'twn_4bit':
      return twn_4bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'twn_6bit':
      return twn_6bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'twn_8bit':
      return twn_8bit(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscv1_q2':
      return dscv1_q2(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscv1_q3':
      return dscv1_q3(fingerprint_input, model_settings,
                      is_training)
  if model_architecture == 'dscv1_q4':
    return dscv1_q4(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'dscv1_q8':
    return dscv1_q8(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'dscv1_q16':
    return dscv1_q16(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'cnnv2':
    return cnnv2(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'dscv1':
    return dscv1(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'dscv2':
    return dscv2(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_bnn_v4':
    return fixed_point_bnn_v4(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_bnn_v3':
    return fixed_point_bnn_v3(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_bnn_v2':
    return fixed_point_bnn_v2(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'FC256_2':
    return FC256_2(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'FC128_3':
    return FC128_3(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'FC128_10':
    return FC128_10(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'FC256_10':
    return FC256_10(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_bnn':
    return fixed_point_bnn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_twn_bnn':
    return fixed_point_twn_bnn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_fwn':
    return fixed_point_fwn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_twn_bntest':
    return fixed_point_twn_bntest(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_twn_test':
    return fixed_point_twn_test(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'group2_conv_twn':
    return group2_conv_twn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'model6':
    return model6(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'model4':
    return model4(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'model5':
    return model5(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_bwn':
    return fixed_point_bwn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_xwn':
    return fixed_point_xwn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_bwn_test':
    return fixed_point_bwn_test(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'fixed_point_twn':
    return fixed_point_twn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'model3':
    return model3(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'hellow_edge_dscnn_test':
    return hellow_edge_dscnn_test(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'hellow_edge_dscnn':
    return hellow_edge_dscnn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'hellow_edge_dscnn_bwn':
    return hellow_edge_dscnn_bwn(fingerprint_input, model_settings,
                                  is_training)
  if model_architecture == 'single_fc':
  ...
  elif model_architecture == 'l_ds_tnn_conv':
    return l_ds_tnn_conv_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'l_ds_bnn_conv_2':
    return l_ds_bnn_conv_2_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'l_ds_bnn_conv_3':
    return l_ds_bnn_conv_3_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'cnn_trad_fpool3':
    return cnn_trad_fpool3(fingerprint_input, model_settings, is_training)
    ...
    return l_allbnn_conv_model(fingerprint_input, model_settings,
                                         is_training)
  elif model_architecture == 'kaggle_2nd':
    return kaggle_2nd_model(fingerprint_input, model_settings,
                                         is_training)
  elif model_architecture == 'kaggle_2nd_narrow':
    return kaggle_2nd_narrow_model(fingerprint_input, model_settings,
                                         is_training)
...
def l_conv_model(fingerprint_input, model_settings, is_training):
  ...
  second_relu = tf.nn.relu(second_conv)
  max_pool2 = tf.nn.max_pool(second_relu, [1, 2, 2, 1], [1, 2, 1, 1], 'SAME')
...
def kaggle_2nd_model(fingerprint_input, model_settings, is_training):
    if is_training:
        dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
    input_frequency_size = model_settings['dct_coefficient_count']
    input_time_size = model_settings['spectrogram_length']
    ...
    # else:
    #   first_dropout = first_relu
    max_pool = tf.nn.max_pool(first_relu, [1, 1, 3, 1], [1, 1, 1, 1], 'SAME')
    print("first_pool weight:" + str(max_pool.get_shape()))
    second_filter_width = 1
    second_filter_height = 7
    ...
    print("second_conv weight:" + str(second_conv.get_shape()))
    second_relu = tf.nn.relu(second_conv)
    max_pool2 = tf.nn.max_pool(second_relu, [1, 1, 4, 1], [1, 1, 1, 1], 'SAME')
    print("second_pool weight:" + str(max_pool2.get_shape()))
    # if is_training:
    #   second_dropout = tf.nn.dropout(second_relu, dropout_prob)
    # else:
    ...
    third_conv = tf.nn.conv2d(max_pool2, third_weights, [1, 1, 1, 1],
                               'VALID') + third_bias
    print("third_conv weight:" + str(third_conv.get_shape()))
    third_relu = tf.nn.relu(third_conv)

    forth_filter_width = 7
    forth_filter_height = 1
    forth_filter_count = 512
    forth_weights = tf.Variable(
        tf.truncated_normal(
            [
                forth_filter_height, forth_filter_width, third_filter_count,
                forth_filter_count
            ],
            stddev=0.01))

    forth_bias = tf.Variable(tf.zeros([forth_filter_count]))
    forth_conv = tf.nn.conv2d(third_relu, forth_weights, [1, 1, 1, 1],
                              'SAME') + forth_bias
    print("forth_conv weight:" + str(forth_conv.get_shape()))
    forth_relu = tf.nn.relu(forth_conv)
    max_pool3 = tf.nn.max_pool(forth_relu, [1, forth_conv.get_shape()[1], 1, 1], [1, 1, 1, 1], 'SAME')
    print("max_pool3 weight:" + str(max_pool3.get_shape()))


    conv_shape = max_pool3.get_shape()
    conv_output_width = conv_shape[2]
    conv_output_height = conv_shape[1]
    filter_count = conv_shape[3]
    conv_element_count = int(
        conv_output_width * conv_output_height *filter_count)
    flattened_conv = tf.reshape(max_pool3,
                                       [-1, conv_element_count])
    print("element count FC:" + str(flattened_conv))
    label_count = model_settings['label_count']
    first_fc_weights = tf.Variable(
        tf.truncated_normal(
            [conv_element_count, 16], stddev=0.01))
    first_fc_bias = tf.Variable(tf.zeros([16]))
    ...
    second_fc_weights = tf.Variable(
        tf.truncated_normal(
            [16, 256], stddev=0.01))
    second_fc_bias = tf.Variable(tf.zeros([256]))
    second_fc = tf.matmul(first_fc, second_fc_weights) + second_fc_bias
    second_fc = tf.nn.relu(second_fc)

    final_fc_weights = tf.Variable(
        tf.truncated_normal(
            [256, label_count], stddev=0.01))
    final_fc_bias = tf.Variable(tf.zeros([label_count]))
    final_fc = tf.matmul(second_fc, final_fc_weights) + final_fc_bias
    weight_num = first_filter_height * first_filter_width * 1 * first_filter_count \
                 + second_filter_height * second_filter_width * first_filter_count * second_filter_count \
                 + third_filter_height * third_filter_width * second_filter_count * third_filter_count \
                 + forth_filter_height * forth_filter_width * third_filter_count * forth_filter_count \
                 + conv_element_count * 16 + 16 * 256  # weights num
    print("Weights number:" + str(weight_num))
    if is_training:
        return final_fc, dropout_prob
    else:
        return final_fc
def kaggle_2nd_narrow_model(fingerprint_input, model_settings, is_training):
    if is_training:
        dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
    input_frequency_size = model_settings['dct_coefficient_count']
    input_time_size = model_settings['spectrogram_length']
    ...
    forth_filter_width = 1
    forth_filter_height = 8
    forth_filter_count = 128
    forth_weights = tf.Variable(
        tf.truncated_normal(
            [
                forth_filter_height, forth_filter_width, third_filter_count,
...
def l_ds_bnn_conv_model(fingerprint_input, model_settings, is_training):
    ...
    first_weights = tf.Variable(
        tf.truncated_normal(
            ...
            stddev=0.1))
    first_weights = binarize(first_weights)
    print("first conv weights:"  + str(first_weights.get_shape()))
    first_conv = tf.nn.conv2d(fingerprint_4d, first_weights, [1, 2, 2, 1], 'SAME')
    print("first conv shape:"  + str(first_conv.get_shape()))

    first_conv = batch_normalization_layer(first_conv, 'first_conv', is_training)  # batchnormalization
    first_relu = tf.clip_by_value(first_conv,-1,1)
    max_pool = tf.nn.max_pool(first_relu, [1, 2, 2, 1], [1, 1, 1, 1], 'SAME')
    print("first max_pool shape:"  + str(max_pool.get_shape()))

    #######    dsconv1   ########
    ds1conv_filter_width = 3
    ds1conv_filter_height = 3
    ds1conv_filter_count = 5
    ds1conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds1conv_filter_height, ds1conv_filter_width, first_filter_count, ds1conv_filter_count], stddev=0.1))
    ds1conv_weights = binarize(ds1conv_weights)
    print("ds1conv_weights :"  + str(ds1conv_weights.get_shape()))

    # depthwise conv
    ds1_conv = tf.nn.depthwise_conv2d(max_pool, ds1conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    print("ds1_conv shape:"  + str(ds1_conv.get_shape()))
    ...
    ds1_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, first_filter_count * ds1conv_filter_count, ds1_out_channels], stddev=0.1))
    print("ds1_pointwise_weights :"  + str(ds1_pointwise_weights.get_shape()))

    ds1_pointwise_weights = binarize(ds1_pointwise_weights)
    ds1_pointwise_conv = tf.nn.conv2d(ds1_relu1, ds1_pointwise_weights, [1, 1, 1, 1], 'SAME')
    print("ds1_pointwise_conv shape :"  + str(ds1_pointwise_conv.get_shape()))
    ...
    ds2conv_filter_width = 3
    ds2conv_filter_height = 3
    ds2conv_filter_count = 5
    ds2conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds2conv_filter_height, ds2conv_filter_width, ds1_out_channels, ds2conv_filter_count], stddev=0.1))
    ds2conv_weights = binarize(ds2conv_weights)
    print("ds2conv_weights  :"  + str(ds2conv_weights.get_shape()))

    # depthwise conv
    ds2_conv = tf.nn.depthwise_conv2d(ds1_relu2, ds2conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    print("ds2_conv shape :"  + str(ds2_conv.get_shape()))
    ...
    ds2_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds1_out_channels * ds2conv_filter_count, ds2_out_channels], stddev=0.1))
    ds2_pointwise_weights = binarize(ds2_pointwise_weights)
    print("ds2_pointwise_weights  :"  + str(ds2_pointwise_weights.get_shape()))

    ds2_pointwise_conv = tf.nn.conv2d(ds2_relu1, ds2_pointwise_weights, [1, 1, 1, 1], 'SAME')
    print("ds2_pointwise_conv shape :"  + str(ds2_pointwise_conv.get_shape()))
    ...
    ds3conv_filter_width = 3
    ds3conv_filter_height = 3
    ds3conv_filter_count = 5
    ds3conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds3conv_filter_height, ds3conv_filter_width, ds2_out_channels, ds3conv_filter_count], stddev=0.1))
    ds3conv_weights = binarize(ds3conv_weights)
    print("ds3conv_weights  :"  + str(ds3conv_weights.get_shape()))

    # depthwise conv
    ds3_conv = tf.nn.depthwise_conv2d(ds2_relu2, ds3conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    print("ds3_conv shape :"  + str(ds3_conv.get_shape()))
    ...
    ds3_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds2_out_channels * ds3conv_filter_count, ds3_out_channels], stddev=0.1))
    ds3_pointwise_weights = binarize(ds3_pointwise_weights)
    print("ds3_pointwise_weights  :"  + str(ds3_pointwise_weights.get_shape()))

    ds3_pointwise_conv = tf.nn.conv2d(ds3_relu1, ds3_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    print("ds3_pointwise_conv shape :"  + str(ds3_pointwise_conv.get_shape()))
    ...
    ds4conv_filter_width = 3
    ds4conv_filter_height = 3
    ds4conv_filter_count = 5
    ds4conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds4conv_filter_height, ds4conv_filter_width, ds3_out_channels, ds4conv_filter_count], stddev=0.1))
    ds4conv_weights = binarize(ds4conv_weights)
    print("ds4conv_weights  :"  + str(ds4conv_weights.get_shape()))

    # depthwise conv
    ds4_conv = tf.nn.depthwise_conv2d(ds3_relu2, ds4conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    print("ds4_conv shape :"  + str(ds4_conv.get_shape()))
    ...
    ds4_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds3_out_channels * ds4conv_filter_count, ds4_out_channels], stddev=0.1))
    ds4_pointwise_weights = binarize(ds4_pointwise_weights)
    print("ds4_pointwise_weights  :"  + str(ds4_pointwise_weights.get_shape()))

    ds4_pointwise_conv = tf.nn.conv2d(ds4_relu1, ds4_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    print("ds4_pointwise_conv shape :"  + str(ds4_pointwise_conv.get_shape()))
    ...
    # average pool
    avg_pool = tf.nn.avg_pool(ds4_relu2, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
    print("avg_pool shape  :"  + str(avg_pool.get_shape()))
    ...
    flattened_avg_pool = tf.reshape(avg_pool, [-1, avg_pool_element_count])
    print("flattened_avg_pool shape  :"  + str(flattened_avg_pool.get_shape()))

    label_count = model_settings['label_count']
    fc_weights = tf.Variable(
        tf.truncated_normal(
            [avg_pool_element_count, label_count], stddev=0.1))
    print("fc_weights  :"  + str(fc_weights.get_shape()))
    ...
    if is_training:
        ...
        return fc
def l_ds_bnn_conv_3_model(fingerprint_input, model_settings, is_training):
    if is_training:
        dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
    input_frequency_size = model_settings['dct_coefficient_count']
    input_time_size = model_settings['spectrogram_length']
    ...
    first_conv = batch_normalization_layer(first_conv, 'first_conv', is_training)  # batchnormalization
    first_relu = tf.clip_by_value(first_conv,-1,1)
    print("first_relu :" + str(first_relu.get_shape()))
    max_pool = tf.nn.max_pool(first_relu, [1, 2, 2, 1], [1, 1, 1, 1], 'SAME')
    #######    dsconv1   ########
    ds1_out_channels_front = 64

    ds1_pointwise_weights_front = tf.Variable(
        tf.truncated_normal(
            [1, 1, first_filter_count, ds1_out_channels_front], stddev=0.1))
    ds1_pointwise_weights_front = binarize(ds1_pointwise_weights_front)
    max_pool = tf.nn.conv2d(max_pool, ds1_pointwise_weights_front, [1, 1, 1, 1], 'SAME')
    ...
    ds1conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds1conv_filter_height, ds1conv_filter_width, first_filter_count, ds1conv_filter_count], stddev=0.1))
    ds1conv_weights = tw_binarize(ds1conv_weights,0,'dsconv1')
    # depthwise conv
    ds1_conv = tf.nn.depthwise_conv2d(max_pool, ds1conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds1_conv = batch_normalization_layer(ds1_conv, 'ds1_conv', is_training)
    ...
    ds1_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, first_filter_count * ds1conv_filter_count, ds1_out_channels], stddev=0.1))
    ds1_pointwise_weights = tw_binarize(ds1_pointwise_weights,0,'ds1_pt')
    ds1_pointwise_conv = tf.nn.conv2d(ds1_relu1, ds1_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds1_pointwise_conv = batch_normalization_layer(ds1_pointwise_conv, 'ds1_pointwise_conv', is_training)
    print("ds1_pointwise_conv :" + str(ds1_pointwise_conv.get_shape()))
    ds1_pointwise_conv = tf.add(ds1_pointwise_conv,first_conv)
    ds1_relu2 = tf.clip_by_value(ds1_pointwise_conv,-1,1)

    #######    dsconv2   ########
    ds2_out_channels_front = 64

    ds2_pointwise_weights_front = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds1_out_channels, ds2_out_channels_front], stddev=0.1))
    ds2_pointwise_weights_front = binarize(ds2_pointwise_weights_front)
    ds1_relu2 = tf.nn.conv2d(ds1_relu2, ds2_pointwise_weights_front, [1, 1, 1, 1], 'SAME')
    ...
    ds2conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds2conv_filter_height, ds2conv_filter_width, ds1_out_channels, ds2conv_filter_count], stddev=0.1))
    ds2conv_weights = tw_binarize(ds2conv_weights,0,'ds2conv')
    # depthwise conv
    ds2_conv = tf.nn.depthwise_conv2d(ds1_relu2, ds2conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds2_conv = batch_normalization_layer(ds2_conv, 'ds2_conv', is_training)
    ...
    ds2_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds1_out_channels * ds2conv_filter_count, ds2_out_channels], stddev=0.1))
    ds2_pointwise_weights = tw_binarize(ds2_pointwise_weights,0,'ds2_pt')
    ds2_pointwise_conv = tf.nn.conv2d(ds2_relu1, ds2_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds2_pointwise_conv = batch_normalization_layer(ds2_pointwise_conv, 'ds2_pointwise_conv', is_training)
    print("ds2_pointwise_conv :" + str(ds2_pointwise_conv.get_shape()))
    ds2_pointwise_conv = tf.add(ds2_pointwise_conv,first_conv)

    ds2_relu2 = tf.clip_by_value(ds2_pointwise_conv,-1,1)

    #######    dsconv3   ########
    ds3_out_channels_front = 64

    ds3_pointwise_weights_front = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds2_out_channels, ds3_out_channels_front], stddev=0.1))
    ds3_pointwise_weights_front = binarize(ds3_pointwise_weights_front)
    ds2_relu2 = tf.nn.conv2d(ds2_relu2, ds3_pointwise_weights_front, [1, 1, 1, 1], 'SAME')
    ...
    ds3conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds3conv_filter_height, ds3conv_filter_width, ds2_out_channels, ds3conv_filter_count], stddev=0.1))
    ds3conv_weights = tw_binarize(ds3conv_weights,0,'ds3conv')
    # depthwise conv
    ds3_conv = tf.nn.depthwise_conv2d(ds2_relu2, ds3conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds3_conv = batch_normalization_layer(ds3_conv, 'ds3_conv', is_training)
    ...
    ds3_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds2_out_channels * ds3conv_filter_count, ds3_out_channels], stddev=0.1))
    ds3_pointwise_weights = tw_binarize(ds3_pointwise_weights,0,'ds3_pt')
    ds3_pointwise_conv = tf.nn.conv2d(ds3_relu1, ds3_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds3_pointwise_conv = batch_normalization_layer(ds3_pointwise_conv, 'ds3_pointwise_conv', is_training)
    print("ds3_pointwise_conv :" + str(ds3_pointwise_conv.get_shape()))
    ds3_pointwise_conv = tf.add(ds3_pointwise_conv,first_conv)

    ds3_relu2 = tf.clip_by_value(ds3_pointwise_conv,-1,1)

    #######    dsconv4   ########
    ds4_out_channels_front = 64

    ds4_pointwise_weights_front = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds3_out_channels, ds4_out_channels_front], stddev=0.1))
    ds4_pointwise_weights_front = binarize(ds4_pointwise_weights_front)
    ds3_relu2 = tf.nn.conv2d(ds3_relu2, ds4_pointwise_weights_front, [1, 1, 1, 1], 'SAME')
    ...
    ds4conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds4conv_filter_height, ds4conv_filter_width, ds3_out_channels, ds4conv_filter_count], stddev=0.1))
    ds4conv_weights = tw_binarize(ds4conv_weights,0,'ds4conv')
    # depthwise conv
    ds4_conv = tf.nn.depthwise_conv2d(ds3_relu2, ds4conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds4_conv = batch_normalization_layer(ds4_conv, 'ds4_conv', is_training)
    ...
    ds4_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds3_out_channels * ds4conv_filter_count, ds4_out_channels], stddev=0.1))
    ds4_pointwise_weights = tw_binarize(ds4_pointwise_weights,0,'ds4_pt')
    ds4_pointwise_conv = tf.nn.conv2d(ds4_relu1, ds4_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds4_pointwise_conv = batch_normalization_layer(ds4_pointwise_conv, 'ds4_pointwise_conv', is_training)
    print("ds4_pointwise_conv :" + str(ds4_pointwise_conv.get_shape()))
    ds4_pointwise_conv = tf.add(ds4_pointwise_conv,first_conv)
    ...
    fc_weights = tf.Variable(
        tf.truncated_normal(
            [avg_pool_element_count, label_count], stddev=0.1))
    fc_weights = tw_binarize(fc_weights,0,'fc')
    fc_bias = tf.Variable(tf.zeros([label_count]))
    fc_bias = tw_binarize(fc_bias,0,'fc_bias')
    fc = tf.add(tf.matmul(flattened_avg_pool, fc_weights), fc_bias)
    #fc = tf.clip_by_value(fc,-1,1)
    weights_num = first_filter_count * ds1conv_filter_count * ds1_out_channels + ds1_out_channels * ds2conv_filter_count * ds2_out_channels \
                  + ds2_out_channels * ds3conv_filter_count * ds3_out_channels + ds3_out_channels * ds4conv_filter_count * ds4_out_channels \
...
def l_ds_tnn_conv_model(fingerprint_input, model_settings, is_training):
    if is_training:
        dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
    input_frequency_size = model_settings['dct_coefficient_count']
    input_time_size = model_settings['spectrogram_length']
    ...
    first_weights = tf.Variable(
        tf.truncated_normal(
            [first_filter_height, first_filter_width, 1, first_filter_count],
            stddev=1),name = "1_weights")
    first_weights = ternary(first_weights)
    print(first_weights.get_shape())
    first_conv = tf.nn.conv2d(fingerprint_4d, first_weights, [1, 2, 2, 1], 'SAME')
    first_conv = batch_normalization_layer(first_conv, 'first_conv', is_training)  # batchnormalization
    first_relu = tf.clip_by_value(first_conv,-1,1)
    ...
    ds1conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds1conv_filter_height, ds1conv_filter_width, first_filter_count, ds1conv_filter_count], stddev=1))
    ds1conv_weights = ternary(ds1conv_weights)
    # depthwise conv
    ds1_conv = tf.nn.depthwise_conv2d(max_pool, ds1conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds1_conv = batch_normalization_layer(ds1_conv, 'ds1_conv', is_training)
    ...
    ds1_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, first_filter_count * ds1conv_filter_count, ds1_out_channels], stddev=1))
    ds1_pointwise_weights = ternary(ds1_pointwise_weights)
    ds1_pointwise_conv = tf.nn.conv2d(ds1_relu1, ds1_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds1_pointwise_conv = batch_normalization_layer(ds1_pointwise_conv, 'ds1_pointwise_conv', is_training)
    ds1_relu2 = tf.clip_by_value(ds1_pointwise_conv,-1,1)
    ...
    ds2conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds2conv_filter_height, ds2conv_filter_width, ds1_out_channels, ds2conv_filter_count], stddev=1))
    ds2conv_weights = ternary(ds2conv_weights)
    # depthwise conv
    ds2_conv = tf.nn.depthwise_conv2d(ds1_relu2, ds2conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds2_conv = batch_normalization_layer(ds2_conv, 'ds2_conv', is_training)
    ...
    ds2_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds1_out_channels * ds2conv_filter_count, ds2_out_channels], stddev=1))
    ds2_pointwise_weights = ternary(ds2_pointwise_weights)
    ds2_pointwise_conv = tf.nn.conv2d(ds2_relu1, ds2_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds2_pointwise_conv = batch_normalization_layer(ds2_pointwise_conv, 'ds2_pointwise_conv', is_training)
    ds2_relu2 = tf.clip_by_value(ds2_pointwise_conv,-1,1)
    ...
    ds3conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds3conv_filter_height, ds3conv_filter_width, ds2_out_channels, ds3conv_filter_count], stddev=1))
    ds3conv_weights = ternary(ds3conv_weights)
    # depthwise conv
    ds3_conv = tf.nn.depthwise_conv2d(ds2_relu2, ds3conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds3_conv = batch_normalization_layer(ds3_conv, 'ds3_conv', is_training)
    ...
    ds3_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds2_out_channels * ds3conv_filter_count, ds3_out_channels], stddev=1))
    ds3_pointwise_weights = ternary(ds3_pointwise_weights)
    ds3_pointwise_conv = tf.nn.conv2d(ds3_relu1, ds3_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds3_pointwise_conv = batch_normalization_layer(ds3_pointwise_conv, 'ds3_pointwise_conv', is_training)
    ds3_relu2 = tf.clip_by_value(ds3_pointwise_conv,-1,1)
    ...
    ds4conv_weights = tf.Variable(
        tf.truncated_normal(
            [ds4conv_filter_height, ds4conv_filter_width, ds3_out_channels, ds4conv_filter_count], stddev=1))
    ds4conv_weights = ternary(ds4conv_weights)
    # depthwise conv
    ds4_conv = tf.nn.depthwise_conv2d(ds3_relu2, ds4conv_weights, strides=[1, 1, 1, 1], padding='SAME')
    # batchnormalization
    ds4_conv = batch_normalization_layer(ds4_conv, 'ds4_conv', is_training)
    ...
    ds4_pointwise_weights = tf.Variable(
        tf.truncated_normal(
            [1, 1, ds3_out_channels * ds4conv_filter_count, ds4_out_channels], stddev=1))
    ds4_pointwise_weights = ternary(ds4_pointwise_weights)
    ds4_pointwise_conv = tf.nn.conv2d(ds4_relu1, ds4_pointwise_weights, [1, 1, 1, 1], 'SAME')
    # batchnormalization
    ds4_pointwise_conv = batch_normalization_layer(ds4_pointwise_conv, 'ds4_pointwise_conv', is_training)
    ds4_relu2 = tf.clip_by_value(ds4_pointwise_conv,-1,1)
    ...
    fc_weights = tf.Variable(
        tf.truncated_normal(
            [avg_pool_element_count, label_count], stddev=1))
    fc_weights = ternary(fc_weights)
    fc_bias = tf.Variable(tf.zeros([label_count]))
    fc_bias = ternary(fc_bias)
    fc = tf.add(tf.matmul(flattened_avg_pool, fc_weights), fc_bias)
    #fc = tf.clip_by_value(fc,-1,1)
    weights_num = first_filter_count * ds1conv_filter_count * ds1_out_channels + ds1_out_channels * ds2conv_filter_count * ds2_out_channels \
                  + ds2_out_channels * ds3conv_filter_count * ds3_out_channels + ds3_out_channels * ds4conv_filter_count * ds4_out_channels \
...
def ternary(x,thre=0.2):
    ...
    return w
def tw_binarize(x,thre,name):

    shape = x.get_shape()

    #thre_x = tf.stop_gradient(tf.reduce_max(tf.abs(x)) * thre)

    w_p = tf.get_variable('Wp'+name, initializer=1.0)
    w_n = tf.get_variable('Wn'+name, initializer=1.0)
    ...
    mask = tf.where(x > thre, tf.ones(shape) * w_p, tf.ones(shape) * w_n)
