...
def main(_):
    ...
    validation_merged_summaries = tf.summary.merge([tf.get_collection(tf.GraphKeys.SUMMARIES, 'accuracy'),
    ...
    for training_step in xrange(start_step, training_steps_max + 1):
        ...
        if (training_step % FLAGS.eval_step_interval) == 0 or is_last_step:
            ...
            tf.logging.info(
                'Best test accuracy before now = %.1f%% (N=%d)' % (best_acc * 100, 6882) + '  at step of ' + str(
                    best_step))
            tf.logging.info('Best test_istrain accuracy before now = %.1f%% (N=%d)' % (
            best_acc_istrain * 100, 6882) + '  at step of ' + str(best_step_istrain))
        ...
        print_line = 'Best test accuracy before now = %.1f%% (N=%d)' % (best_acc * 100, 6882) + '  at step of ' + str(
            best_step) + '\n' + \
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--summaries_dir',
        type=str,
        default='tmp/retrain_logs_lyc/twn_test_mixup',
