...
class AudioProcessor(object):
  ...
  def __init__(self, data_url, data_dir, silence_percentage, unknown_percentage,
   ...
   # self.save_my_test_file(model_settings)
  ...
  def prepare_data_index(self, silence_percentage, unknown_percentage,
    ...
    for set_index in ['validation', 'testing', 'training']:
      ...
      set_size = len(self.data_index[set_index])
      unknown_size = int(math.ceil(set_size * 7))   #0.7
      ...
      print(set_index +" has  " + str(len(self.seven_to_one_set[set_index])) + ' data,with ' + str(self.unknown_num[set_index]) +' fillers')
  ...
  def get_data_my(self, how_many, offset, model_settings, background_frequency,
    ...
    # random.shuffle(self.unknown_index[mode])
    # # unknown_size = len(self.unknown_index[mode])
    # unknown_size = int(math.ceil(len(candidates) * unknown_percentage / 100))
    # candidates.extend(self.unknown_index[mode][:unknown_size])
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      if time_shift > 0:
        time_shift_amount = np.random.randint(-time_shift, time_shift) #生成-100 到100的随机数
      else:
        time_shift_amount = 0
  ...
  def get_data(self, how_many, offset, model_settings, background_frequency,
    ...
    # random.shuffle(self.unknown_index[mode])
    # unknown_size = int(math.ceil(set_size * unknown_percentage / 100))
    # self.data_index[mode].extend(unknown_index[mode][:unknown_size])
    candidates = self.data_index[mode]
    if how_many == -1:
  ...
  # def get_nosilence_data(self, model_settings, background_frequency,
  #              background_volume_range, mode, sess):
  #   frame_count = 41
  #   sample_count = 10
  #   time_shift_ms = 10
  #   sample_rate = 16000
  ...
  #   # print("window_stride_samples:" + str(window_stride_samples))
  #   window_size_samples = model_settings['window_size_samples']
  #   # print("window_size_samples:" + str(window_size_samples))
  #   desired_samples = model_settings['desired_samples']
  ...
  #   use_background = self.background_data and (mode == 'training')
  #   pick_deterministically = (mode != 'training')
  #   sample_class = {'clip_samples': [], 'one_sample_count': [], 'label': [], 'file': []}
  #   frame_size_samples = (frame_count - 1) * window_stride_samples + window_size_samples
  #   # print("frame_size_samples:" + str(frame_size_samples))
  #   sum_samples = 0
  #   for i in xrange(0, sample_count):  # sample count =3
  #     rate = np.random.random()
  #     if rate < 0.25:
  #       sample_index = np.random.randint(len(self.unknown_index[mode]))
  #       one_sample = self.unknown_index[mode][sample_index]
  #     else:
  #       sample_index = np.random.randint(len(candidates))
  #       one_sample = candidates[sample_index]
  #     # print(sample_index)
  #     # print(one_sample)
  #     with contextlib.closing(wave.open(one_sample['file'], 'rb')) as wf:
  #       clip_samples = wf.getnframes()
  #     one_sample_count = int((clip_samples - frame_size_samples) / window_stride_samples)
  #     if not one_sample_count > 0:
  #       one_sample_count = 1
  #     # print("one sample amount:" + str(one_sample_count))
  #     sum_samples = sum_samples + one_sample_count
  #     sample_class['clip_samples'].append(clip_samples)
  #     sample_class['one_sample_count'].append(one_sample_count)
  #     sample_class['label'].append(one_sample['label'])
  #     sample_class['file'].append(one_sample['file'])
  #   # print("one batch amount:" + str(sum_samples))
  #   data = np.zeros((sum_samples, model_settings['fingerprint_size']))
  #   labels = np.zeros((sum_samples, model_settings['label_count']))
  #   # clip_samples = int(16000 * 1000 / 10000
  #   # print("sample-class clip-samples:" + str(sample_class['clip_samples']))
  #   # print("sample-class one_samples_count:" + str(sample_class['one_sample_count']))
  #   # print("sample-class label:" + str(sample_class['label']))
  #   # print("sample-class file:" + str(sample_class['file']))
  #   # print("batch shape:" + str(data.shape))
  #   # print("label shape:" + str(labels.shape))
  #   for i in xrange(0, sample_count):
  #     time_shift_padding = [[0, 0], [0, 0]]
  #     for j in xrange(0, sample_class['one_sample_count'][i]):
  #       time_shift_offset = [time_shift * j, 0]
  #       input_dict = {
  #         self.wav_filename_placeholder_: sample_class['file'][i],
  #         self.time_shift_padding_placeholder_: time_shift_padding,
  #         self.time_shift_offset_placeholder_: time_shift_offset,
  #       }
  #       if use_background:
  ...
  #       input_dict[self.background_volume_placeholder_] = background_volume
  #
  #       if sample_class['label'][i] == SILENCE_LABEL:
  #         input_dict[self.foreground_volume_placeholder_] = 0
  #       else:
  #         input_dict[self.foreground_volume_placeholder_] = 1
  #
  #       # filename= 'my_no_silence/test_' + str(i)+'_'+ str(j)+'.wav'
  #       # input_dict[audio_processor.filename_]=filename
  #       # sess.run(audio_processor.wav_saver,feed_dict=input_dict)
  #       batch_index = sum(sample_class['one_sample_count'][:i])
  #       data[batch_index + j, :] = sess.run(self.mfcc_, feed_dict=input_dict).flatten()
  #       label_index = self.word_to_index[sample_class['label'][i]]
  #       labels[batch_index + j, label_index] = 1
  #   return data,labels,sample_class['one_sample_count']
  #
