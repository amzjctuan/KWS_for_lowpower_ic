...
def fingerprints_to_binary(fingerprints_in,time_size,frequence_size):
    ...
    x = x.astype(np.uint8)
    x = np.reshape(x, [-1, time_size, frequence_size])
    x = x[:, :, :, np.newaxis]
    x = np.array(x, dtype=np.uint8)
    x = np.unpackbits(x, axis=3)
    return x

def onehot(x,time_size,frequence_size):
    x = np.round(x)
...
##############################################
  ...
  fingerprint_input = tf.placeholder(
      tf.float32, [None, model_settings['spectrogram_length'],model_settings['dct_coefficient_count'],100], name='fingerprint_input')
###############################################################################################################################
  # ############ 模型创建 ##########
...
###
  ...
  for training_step in xrange(start_step, training_steps_max + 1):
    ...
    train_fingerprints = onehot(train_fingerprints,model_settings['spectrogram_length'],model_settings['dct_coefficient_count'])
    ...
    if (training_step % FLAGS.eval_step_interval) == 0 or is_last_step:
      ...
      for i in xrange(0, set_size, FLAGS.batch_size):
        ...
        validation_fingerprints = onehot(validation_fingerprints, model_settings['spectrogram_length'],
      ...
      test_fingerprints = onehot(test_fingerprints, model_settings['spectrogram_length'],
                                                       model_settings['dct_coefficient_count'])
  ...
  #                                                          6882))### data_url              下载地址        default='http://download.tensorflow.org/data/speech_commands_v0.01.tar.gz'
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      default='/home/zhangs/zs_data_cut/tmp/data_agc/',
