...
##############################################
  ...
  test_summaries = tf.summary.merge([tf.get_collection(tf.GraphKeys.SUMMARIES,'accuracy'),tf.get_collection(tf.GraphKeys.SUMMARIES,'cross_entropy')])
...
###
  ...
  for training_step in xrange(start_step, training_steps_max + 1):
    ...
    #######       自动切换学习率      #######
    if training_step <13000+1:
        learning_rate_value = learning_rates_list[0]*0.005**(training_step/13000)
    else:
        learning_rate_value = learning_rates_list[0]*0.005   #0.015 12000
    ...
    if (training_step % FLAGS.eval_step_interval) == 0 or is_last_step:
      ...
      final_summary,test_accuracy, conf_matrix,cross_entropy_meantest = sess.run(
          [test_summaries,evaluation_step, confusion_matrix,cross_entropy_mean],
          feed_dict={
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--summaries_dir',
      type=str,
      default='tmp/retrain_logs_lyc/twowords_dscnn_bnn_datav2',
  ...
  parser.add_argument(
      '--train_dir',
      type=str,
      default='tmp/speech_commands_train/twowords_dscnn_bnn_datav2',
