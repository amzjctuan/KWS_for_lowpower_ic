...
###
  ...
  for training_step in xrange(start_step, training_steps_max + 1):
    ...
    if (training_step % FLAGS.eval_step_interval) == 0 or is_last_step:
      ...
      total_accuracy = 0
      total_conf_matrix = None
      set_size = len(audio_processor.seven_to_one_set['testing'])#
      tf.logging.info('set_size=%d', set_size)
      for ii in xrange(0,set_size-100,100):
          test_fingerprints, test_ground_truth = audio_processor.get_data_my(
            -2, ii, model_settings, 0.0, 0.0, 0, 'testing', sess)
          ...
          total_accuracy += (test_accuracy) / int(set_size/100)
          if total_conf_matrix is None:
              total_conf_matrix = conf_matrix
          else:
              total_conf_matrix += conf_matrix
      ...
      # test_acc = tf.summary.scalar('accuracy', tf.convert_to_tensor(total_accuracy))
      # sess.run(test_acc)
      # final_summary = tf.summary.merge([test_acc.eval()])
      # sess.run(final_summary)
      # test_writer.add_summary(test_acc, training_step)
      tf.logging.info('Confusion Matrix:\n %s' % (total_conf_matrix))
      tf.logging.info('test accuracy = %.1f%% (N=%d)' % (total_accuracy * 100,
                                                         set_size))
      tf.logging.info('Best test accuracy before now = %.1f%% (N=%d)' % (best_acc * 100,set_size) + '  at step of ' + str(best_step))
